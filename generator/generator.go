...
type Generate func(bias constraints.Bias) (arbitrary.Arbitrary, shrinker.Shrinker)
...
type Generator func(target reflect.Type, r Random) (Generate, error)
...
func (generator Generator) Map(mapper interface{}) Generator {
	return func(target reflect.Type, r Random) (Generate, error) {
		...
		generate, err := generator(val.Type().In(0), r)
		if err != nil {
...
func (generator Generator) Filter(predicate interface{}) Generator {
	return func(target reflect.Type, r Random) (Generate, error) {
		generate, err := generator(target, r)
...
func (generator Generator) Bind(binder interface{}) Generator {
	return func(target reflect.Type, r Random) (Generate, error) {
		...
		generate, err := generator(binderVal.Type().In(0), r)
		if err != nil {
			return nil, fmt.Errorf("failed to create base generator: %s", err)
		}

		return func(bias constraints.Bias) (arbitrary.Arbitrary, shrinker.Shrinker) {
			sourceArb, sourceShrinker := generate(bias)

			boundGenerator := binderVal.Call([]reflect.Value{sourceArb.Value})[0].Interface().(Generator)
			generate, err = boundGenerator(target, r)
			if err != nil {
				panic(fmt.Errorf("generator composition failed: %s", err))
			}

			boundVal, boundShrinker := generate(bias)

...
type Arbitrary func(target reflect.Type, bias constraints.Bias, r Random) (Generator, error)
...
// it has one output value which must be an Arbitrary. Bind allows using the
// generated value of one generator as an input for another generator.
func (arb Arbitrary) Bind(binder interface{}, retries uint) Arbitrary {
	return func(target reflect.Type, bias constraints.Bias, r Random) (Generator, error) {
		...
		case t.Out(0) != reflect.TypeOf(Arbitrary(nil)):
			return nil, fmt.Errorf("binder's output value must be generator.Arbitrary")
		}

		generator, err := arb(binderVal.Type().In(0), bias, r)
		if err != nil {
			return nil, fmt.Errorf("failed to create base generator")
		}
		sourceVal, sourceShrinker := generator()

		outputs := binderVal.Call([]reflect.Value{sourceVal})
		generator, err = outputs[0].Interface().(Arbitrary)(target, bias, r)
		if err != nil {
			return nil, fmt.Errorf("generator composition failed: %s", err)
		}

		return func() (reflect.Value, shrinker.Shrinker) {
			destinationVal, destinationShrinker := generator()

			shrinkerBinder := func(source reflect.Value) (reflect.Value, shrinker.Shrinker, error) {
				outputs := binderVal.Call([]reflect.Value{source})
				generator, err = outputs[0].Interface().(Arbitrary)(target, bias, r)
				if err != nil {
					return reflect.Value{}, nil, fmt.Errorf("generator binding failed: %s", err)
				}
			...
			return destinationVal, sourceShrinker.
				Retry(retries, retries, sourceVal).
				Bind(shrinkerBinder, destinationShrinker)
		}, nil
	}
}

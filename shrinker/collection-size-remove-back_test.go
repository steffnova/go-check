...
func TestCollectionSizeRemoveBack(t *testing.T) {
	testCases := map[string]func(*testing.T){
		"IndexOutOfUpperBound": func(t *testing.T) {
			...
			arb.Shrinker = CollectionSizeRemoveBack(len(arb.Elements))
		...
		"RemoveLastElement": func(t *testing.T) {
		...
		"Remove2ndElementFromBack": func(t *testing.T) {
			elements := make([]arbitrary.Arbitrary, 10)
			for index := range elements {
				elements[index] = arbitrary.Arbitrary{
					Value:    reflect.ValueOf(uint64(index)),
			...
			arb.Elements = append(arb.Elements[:len(elements)-2], arb.Elements[len(elements)-1])
